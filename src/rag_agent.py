...
class RAGChatbotAgent:
    ...
    def answer_question(self, query: str) -> Dict[str, Any]:
        ...
        if not self.is_loaded:
            return {
                "answer": "The vector store could not be loaded. Please run the builder script first.",
                "source_documents": [],
            }
        ...
        retrieved_docs = (
            self.vector_store_manager.faiss_db.as_retriever().get_relevant_documents(
                query
            )
        )

        return {"answer": result["result"], "source_documents": retrieved_docs}
