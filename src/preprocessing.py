...
def clean_text(text):
    """
    Cleans a single text string by lowercasing, removing special characters, and stopwords.
    """
...
def filter_and_clean_data(df):
    """
    Filters the DataFrame for specified products and cleans the consumer narratives.
    """
    if df is None:
        return None
